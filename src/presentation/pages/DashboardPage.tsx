...
export function DashboardPage() {
	...
	const [activeTab, setActiveTab] = useState<"executed" | "planned">(
		"executed",
	);
	...
	// Toast de éxito desde navegación
	useEffect(() => {
	...
	const fetchData = async () => {
		try {
			...
			// 3. Cargar Hallazgos si hay projectId (Cumplimiento de Rediseño UX)
			if (projectId && metricsData.length > 0) {
				const { data: findings } = await supabase
					.from("findings")
					.select("*")
					.in(
						"test_plan_id",
						metricsData.map((m) => m.test_plan_id),
					)
	...
	const generatePDF = async (planId: string) => {
		...
		const fecha = details.test_date
			? new Date(details.test_date).toLocaleDateString("es-EC", {
					year: "numeric",
					month: "long",
					day: "numeric",
				})
		...
		doc.setFontSize(7);
		doc.setFont("helvetica", "bold");
		doc.text(details.method?.toUpperCase() || "TEST", pageWidth - 37, 52, {
			align: "center",
		});
		...
		const successCount =
			details.observations?.filter((o) => o.success).length || 0;
		const successRate =
			totalObs > 0 ? Math.round((successCount / totalObs) * 100) : 0;
		const totalErrors =
			details.observations?.reduce(
				(acc, o) => acc + (o.errors_count || 0),
				0,
			) || 0;
		const avgTime =
			totalObs > 0
		...
		const kpis: {
		...
		}[] = [
			{
				label: "TASA DE ÉXITO",
				val: `${successRate}%`,
				sub: "Efectividad — tareas completadas correctamente",
				color: C.green,
				bg: [240, 253, 244],
			},
			{
				label: "TIEMPO PROM.",
				val: `${avgTime}s`,
				sub: "Eficiencia — recursos invertidos promedio",
				color: C.amber,
				bg: [255, 251, 235],
			},
			{
				label: "ERRORES",
				val: `${totalErrors}`,
				sub: "Conformidad — fricciones críticas detectadas",
				color: C.red,
				bg: [254, 242, 242],
			},
			{
				label: "OBSERVACIONES",
				val: `${totalObs}`,
				sub: "Validez — volumen de datos registrados",
				color: C.primary,
				bg: [239, 246, 255],
			},
		];
		...
		doc.text(
			"基准  ISO 9241-11:1998  •  Usabilidad: efectividad, eficiencia y satisfacción en el contexto de uso",
			14,
			currentY,
		);
		currentY += 10;
		...
		autoTable(doc, {
			...
			body: [
				[
					"Producto / Módulo",
					`${details.product_name} / ${details.module_name || "N/A"}`,
				],
			...
			columnStyles: {
				0: { fontStyle: "bold", textColor: C.slate900, cellWidth: 46 },
			},
		...
		autoTable(doc, {
			...
			body: [
				[
					"Facilidad — Puntos Positivos",
					details.closing_easy || "No registrado",
				],
				[
					"Confusión — Puntos de Fricción",
					details.closing_confusing || "No registrado",
				],
		...
		for (let i = 1; i <= pageCount; i++) {
			...
			doc.setFont("helvetica", "normal");
			doc.text(
				"Usability Dashboard  •  KPIs estándar ISO 9241-11  •  Página " +
					i +
					" de " +
					pageCount,
				pageWidth / 2,
				...
				observations:
					details.observations && details.observations.length > 0
						? details.observations.map(
								(
									o: Observation & { participants?: Participant; tasks?: Task },
								): ObservationDraft => ({
	...
	return (
		<div className="flex flex-col min-h-full">
			{/* Implementación Funcional UX: Breadcrumbs Dinámicos */}
					...
					<div className="bg-white rounded-3xl shadow-2xl max-w-md w-full overflow-hidden animate-in zoom-in-95 duration-200">
						<div className="p-8 text-center">
							...
							<p className="text-sm text-slate-700 mb-8 leading-relaxed font-medium">
								Esta acción no se puede deshacer de forma sencilla desde el
								panel.
							</p>
							<div className="flex flex-col sm:flex-row gap-3">
								...
								<Button
									...
									disabled={isDeleting}
									className="flex-1 rounded-xl py-6 bg-red-600 hover:bg-red-700 text-white font-semibold transition-colors"
								>
									{isDeleting ? (
				...
				<div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
					<div className="flex-1">
						...
						<p className="mt-1 text-slate-700 max-w-2xl font-medium">
							Panel de instrumentos para la visualización global de métricas y
							KPIs de la misión.
						</p>
					</div>
				...
				<div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-4">
					<div className="flex bg-slate-100 p-1.5 rounded-2xl border border-slate-200 w-fit">
						...
						>
							...
							Misiones Ejecutadas
							<span
								className={`ml-1 px-2 py-0.5 rounded-full text-[10px] ${activeTab === "executed" ? "bg-primary text-white" : "bg-slate-200 text-slate-500"}`}
							>
						...
						>
							...
							Planificadas
							<span
								className={`ml-1 px-2 py-0.5 rounded-full text-[10px] ${activeTab === "planned" ? "bg-primary text-white" : "bg-slate-200 text-slate-500"}`}
							>
								{plannedPlans.length}
							</span>
						</button>
					...
					<div className="bg-white px-5 py-3 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-4">
						...
						<div>
							...
							<p className="text-xl font-bold text-slate-900 leading-tight">
								{metrics.length} Proyectos
							</p>
						</div>
					</div>
				</div>

				{/* Sección de Hallazgos Recientes (Cumplimiento de Rediseño UX) */}
				{projectId && recentFindings.length > 0 && (
					<section className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-700">
						<div className="px-8 py-6 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
							<div className="flex items-center gap-3">
								<AlertTriangle className="text-amber-500" size={20} />
								<h2 className="font-bold text-lg text-slate-900 tracking-tight">
									Hallazgos Recientes del Proyecto
								</h2>
							</div>
						...
						<div className="p-8">
							<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
								{recentFindings.map((f, fi) => (
									<div
										key={fi}
										className="flex items-center justify-between p-5 bg-slate-50 rounded-2xl border border-slate-100 hover:border-primary/20 transition-all group"
									>
										...
										<Button
											variant="ghost"
											size="sm"
											className="text-primary font-bold text-xs group-hover:translate-x-1 transition-transform shrink-0"
										>
											DETALLE <ArrowRight size={12} className="ml-1" />
										</Button>
									</div>
								))}
							</div>
					...
					<div className="flex flex-col items-center justify-center py-20 text-center border-2 border-dashed border-slate-300 rounded-2xl bg-white">
						...
						<h2 className="text-lg font-semibold text-slate-900">
							{searchTerm
								...
								: activeTab === "executed"
									? "No hay misiones ejecutadas aún"
									: "No tenés misiones planificadas"}
						</h2>
						{!searchTerm && activeTab === "planned" && (
							<Button
								onClick={handleNewPlan}
								variant="outline"
								className="mt-4 border-slate-300 font-semibold"
							>
								Planificar mi primera misión
							</Button>
						)}
					</div>
				) : (
							...
							<div className="flex flex-col md:flex-row md:items-center justify-between border-b border-slate-100 pb-4 gap-4">
								<div className="text-left">
									<div className="flex items-center gap-3">
										<h2 className="text-xl font-bold text-slate-900 flex items-center gap-2">
											<span
												className={`h-6 w-1.5 rounded-full ${activeTab === "executed" ? "bg-primary" : "bg-slate-300"}`}
												aria-hidden="true"
											></span>
								...
								<div className="flex items-center gap-3 mt-2 md:mt-0">
									...
									{activeTab === "executed" ? (
										<Button
											size="sm"
											onClick={() => generatePDF(m.test_plan_id)}
											className="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-5 shadow-lg shadow-blue-100"
										>
										...
										<Button
											...
											className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold px-5 shadow-lg shadow-emerald-100"
										>
									...
									<DropdownMenu.Root>
										...
										<DropdownMenu.Portal>
											...
											>
												<DropdownMenu.Item
													onClick={() =>
														navigate(`/dashboard/test-plan/${m.test_plan_id}`)
													}
									...
									{[
										{
											label: "Tasa de Éxito",
											val:
												activeTab === "executed" ? `${m.success_rate}%` : "—",
											icon: TrendingUp,
											color:
												activeTab === "executed"
													? "text-green-700"
										...
										>
											...
											<p
												className="mt-auto pt-1 text-[8px] text-slate-300 font-medium truncate"
												aria-hidden="true"
											>
												{stat.isoRef}
											</p>
